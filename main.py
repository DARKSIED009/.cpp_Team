...
from models import (
    AIExplainRequest,
    AICritiqueRequest,
    AILenderSummaryRequest,
    AISimulateAdviceRequest,
    AIChatRequest,
    AICoachRequest,
    AIAnomalyRequest,
    AILenderNarrativeRequest
)
...
@app.post("/ai/explain-score")
def ai_explain_score(req: AIExplainRequest):
    result = explain_score(req.worker_data)
    return {"text": result}

@app.post("/ai/critique")
def ai_critique(req: AICritiqueRequest):
    result = critique_worker(req.worker_data)
    return {"text": result}

@app.post("/ai/coach")
def ai_coach(req: AICoachRequest):
    result = get_coach_plan(req.worker_data)
    return {"text": result}

@app.post("/ai/anomaly")
def ai_anomaly_report(req: AIAnomalyRequest):
    result = get_anomaly_report(req.worker_data)
    return {"text": result}

@app.post("/ai/narrative")
def ai_lender_narrative(req: AILenderNarrativeRequest):
    result = get_narrative(req.worker_data)
    return {"text": result}

@app.post("/ai/lender-summary")
def ai_lender_summary(req: AILenderSummaryRequest):
    result = get_lender_summary(req.worker_data)
    return {"text": result}

@app.post("/ai/simulate-advice")
def ai_simulate_advice(req: AISimulateAdviceRequest):
    result = get_simulate_advice(req.old_score, req.new_score, req.adjustments)
    return {"text": result}

@app.post("/ai/chat")
def ai_chat(req: AIChatRequest):
    history_dicts = [{"role": m.role, "parts": [{"text": m.text}]} for m in req.history]
    result = chat_with_bot(req.worker_data, history_dicts, req.message)
    return {"text": result}
